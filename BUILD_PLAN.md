...
> Corrections applied: `.py` → `.ts`; model per C2; Extract Template → `nimble_extract` per C1; plugin beat per C3: pending Devansh (Isha proposes Veto's own MCP session instead).
