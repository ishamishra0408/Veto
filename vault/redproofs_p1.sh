#!/usr/bin/env bash
# Phase 1 red proofs. Run: bash vault/redproofs_p1.sh
cd "$(dirname "$0")/.." || exit 2
NODE="node --disable-warning=ExperimentalWarning"
fail=0
$NODE vault/ship.ts >/dev/null || { echo "setup: gated ship failed"; exit 2; }
$NODE vault/verify_pins.ts >/dev/null || { echo "setup: baseline verify not clean"; exit 2; }

# R1 — the seam is real
out=$(grep -rn "MockAdapter" vault/ --include=*.ts --exclude-dir=node_modules | grep -v adapters.ts | grep -v redproofs)
...
pid=$(grep -o '\[pin:[0-9a-f]*\]' vault/report.md | head -1 | sed 's/\[pin:\(.*\)\]/\1/')
before=$(shasum -a 256 vault/pins.db | cut -d' ' -f1)
tmp=$(mktemp -t pins.XXXXXX); cp vault/pins.db "$tmp"
...
vout=$($NODE vault/verify_pins.ts --db "$tmp"); vrc=$?
rm -f "$tmp"
after=$(shasum -a 256 vault/pins.db | cut -d' ' -f1)
